...
async fn main() -> anyhow::Result<()> {
    ...
    async fn heartbeat_billboard(perfmon_tx: Sender<PerfmonCommand>, console_tx: Sender<ConsoleCommand>) -> () {

        let v = join!(
            perfmon_tx.send(PerfmonCommand::Tick),
            console_tx.send(ConsoleCommand::Print(ConsoleMessage::new("hi".to_string() + " " + &*random::<u8>().to_string()), false))
        );
    ...
    scheduler
        .every(5.seconds())
        .run(move || heartbeat_billboard(perfmon_tx.clone(), neutral_con_tx.clone()));
    ...
    //DISCORD STUFF
    let token: String = {
        core.config.discord_token.clone()
    };
